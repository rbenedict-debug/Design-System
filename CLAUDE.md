...
| `ds-column-panel` | Custom | No Material equivalent — AG Grid custom tool panel; sections: density toggle, Column Visibility (collapsible), pivot mode, row groups (active chips + inline picker menu), values (active chips + inline picker menu); width 300px; shown via `[data-panel-open]` on container; whole panel scrolls via `overflow-y: auto` |
