...
- Never put `ds-dashboard-toolbar` inside `ds-page-content__main--dashboard` — it must be a sibling flex child between `__heading` and `__main--dashboard` so it stays fixed while tile content scrolls; the page title goes in `__heading` as a normal visible H1
- Never remove `overflow-x: clip` and `overflow-clip-margin: 12px` from `ds-page-content__main--dashboard` — these allow tile box-shadows to render outside the scroll container's border box without adding layout padding; `clip` is exempt from the CSS coercion rule that would normally force `overflow-x` to `auto` when `overflow-y: auto` is set
