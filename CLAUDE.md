...
- Dashboard pages require three companion classes together — never use `__main--dashboard` alone: (1) add `ds-page-content--dashboard` to `ds-page-content` (`overflow: hidden` — clips the flex column, not a scroll container); (2) wrap heading + toolbar in `ds-page-content__dashboard-header` (static block above the scroller — no sticky needed); (3) use `ds-page-content__main--dashboard` for the tile canvas (`overflow-x: clip; overflow-y: auto` — this IS the scroll container; tile box-shadows are never clipped because they scroll with the tiles)
