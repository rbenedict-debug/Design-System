...
| `onfloTheme` | n/a — AG Grid theme object | Pre-configured AG Grid theme. Consumers set `theme: onfloTheme` in gridOptions. **Requires `ag-grid-community ^33`** — uses v31+ programmatic Theme API (`themeQuartz.withPart().withParams()`); will crash at runtime on older versions. Built on Quartz + `iconSetMaterial`. Static sizing params (headerHeight 56px fixed, rowHeight **68px** comfortable default / 42px compact, iconSize 20, spacing 8) set via `withParams()`; color/font/border tokens applied via CSS custom properties in `_table-ag-theme.scss`. AG Grid's native borders disabled — custom renderers own all border rendering. Do **not** load `ag-theme-quartz.css` in consuming projects — conflicts with the programmatic theme. |
...
| `ds-dashboard-toolbar` | Custom | No Material equivalent — page-level toolbar for dashboards. **Placement:** sibling flex child inside `ds-page-content`, between `ds-page-content__heading` and `ds-page-content__main--dashboard` — it does NOT go inside `__main--dashboard`. This keeps it fixed while only the tile content scrolls. No enclosing box — each control is an individually elevated element (same `box-shadow` as metric cards, no borders). **Left:** `__identity` block with `__title` + optional `__subtitle` (bare text on canvas). **Right:** `__controls` row containing projected `[toolbar-filters]` quick-filter controls + `__btn` icon buttons for filter and more. Inputs: `[title]`, `[subtitle]`, `[filterActive]` (boolean — marks filter button active: brand bg + filled `filter_alt` icon). Outputs: `(filterClick)`, `(moreClick)`. Quick filters (e.g. `__date-select`) use the `data-mouse-focus` focus-ring suppression pattern. `__btn.is-active` marks the filter button as toggled; bind via `[filterActive]` on the Angular component. Filter icon is outlined by default, filled only when active. |
...
- Dashboard pages require three companion classes together — never use `__main--dashboard` alone: (1) add `ds-page-content--dashboard` to `ds-page-content` (`gap: 0` — no overflow; the canvas uses negative margins); (2) wrap heading + toolbar in `ds-page-content__dashboard-header` (static block above the scroller — no sticky needed; its `padding-bottom: 16px` is the sole source of the toolbar-to-tiles gap); (3) use `ds-page-content__main--dashboard` for the tile canvas — it is the scroll container (`overflow-x: clip; overflow-y: auto`); it has `padding: 0 16px 16px 16px` and `margin: 0 -16px -16px -16px` to cancel the body's and parent's padding and produce a true uniform 16px spacing from all edges; the 16px left padding also keeps tile box-shadows inside the padding-box so `overflow-x: clip` never clips them
