...
- Never remove the `padding: 4px 8px 8px` from `ds-page-content__main--dashboard` — `overflow-y: auto` coerces `overflow-x` to `hidden`, and the padding is what prevents the scroll container from clipping metric card and chart tile box-shadows at the left/right/bottom edges
