...
- Tightened peer dependency ranges. Angular ecosystem (`@angular/*`,
  `@angular/cdk`, `@angular/material`) moved from open `>=21.0.0` to
  `^21.0.0` — patches and minors flow through, but Angular 22+ majors are
  blocked (they always introduce breaking changes). `ag-grid-community`
  pinned to `>=33.0.0 <36.0.0` to match the range actually tested.
  `highcharts` moved to `^11.0.0`.
- `ag-grid-community` and `highcharts` marked as optional peer dependencies
  via `peerDependenciesMeta`. Design-team projects (which don't use AG Grid
  or Highcharts) will no longer see install errors for them — npm will
  silently skip the missing-peer check.
